...
namespace AdvantShop.Module.OneSApi.Handlers.Client
{
    public class ProductListHandler
    {
        ...
        private readonly int? _salesType;
        private readonly bool _recomended;
        ...
        public ProductListHandler(EProductOnMain type, bool inDepth, CategoryModel filter, int? productListId, bool isMobile, int? salesType, bool recomended)
        {
            _type = type;
            _inDepth = inDepth;
            _filter = filter;
                ...
                default:
                    switch (_type)
                    {
                        ...
                        case EProductOnMain.None:
                            //if (_recommended)
                            //    _paging.OrderBy("SortRecommended".AsSqlField("Sorting"))
                            //           .OrderByDesc("Product.ProductID".AsSqlField("ProductIDSorting"));
                            //else
                                _paging.OrderByDesc("Product.ProductID".AsSqlField("ProductIDSorting"));
                            break;
        ...
        private void BuildFilter()
        {
            ...
            else if (_type == EProductOnMain.None && _recomended)
            {
            ...
            if (_filter.PriceFrom.HasValue || _filter.PriceTo.HasValue)
            {
                ...
                _paging.Where("and (ProductExt.PriceTemp>= {0} ", priceFrom * currency.Rate)
                       .Where("and ProductExt.PriceTemp <= {0})", priceTo * currency.Rate);
                hasFilter = true;
            }
        ...
        private void BuildExcludingFilters()
        {
            ...
            if (_filter.CategoryId != 0 && SettingsCatalog.ShowPropertiesFilterInProductList)
            {
                var task = Task.Run(() =>
                {
        ...
        public ProductListPagingModel GetForFilter()
        {
            ...
            _model.ProductIds = _paging.ItemsIds<int>("Product.ProductID");
